        ...
        if (ends("\03" "eed")) {
            ...
            if ((ends("\02" "ed") || ends("\03" "ing")) && vowelinstem()) {
                ...
                if (ends("\02" "at")) setto("\03" "ate");
                    ...
                    if (ends("\02" "bl")) setto("\03" "ble");
                    else
                        if (ends("\02" "iz")) setto("\03" "ize");
                        else
                            if (doublec(k)) {
                                k--;
                                {
...
string Stemming::stem(const string & input)
{
    string ret(input);
    if (!ret.empty())
    {
        char temp[1001];
        strncpy(temp, input.data(),1000);
        int len = input.length() < 1000 ? input.length() : 1000;
        int end = stemming::stem(temp, 0, len - 1);
        temp[end+1] = '\0';
        ret.assign(temp);
    }
    return ret;
